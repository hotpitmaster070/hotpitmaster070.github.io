...
async function getContext(){
  ...
  userId = user.id
  if(!restaurantId) throw new Error('Нет restaurant_id в user_metadata')
}
...
export function renderPassportView(){
  ...
  return `
    ...
    <div id="templatesModal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.7);backdrop-filter:blur(4px);z-index:50;align-items:center;justify-content:center">
      <div style="background:#18181b;border:1px solid #2a2a2a;border-radius:20px;padding:24px;width:95%;max-width:640px;max-height:85vh;overflow:auto">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px">
          <h3 style="color:var(--accent);margin:0;font-size:20px">Мои заготовки</h3>
          <button id="closeTemplatesBtn" style="background:transparent;border:0;font-size:28px;color:#888;cursor:pointer">×</button>
        </div>

        <div id="templatesList" style="display:flex;flex-direction:column;gap:10px;margin-bottom:24px"></div>

        <h4 style="margin:0 0 12px;color:#d4d4d8;font-size:15px">Добавить позицию</h4>
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:10px">
          <input id="tplTitle" placeholder="Название заготовки" style="padding:12px;border-radius:10px;border:1px solid #333;background:#0f0f0f;color:#fff">
          <input id="tplQty" type="number" step="0.1" placeholder="Кол-во" style="padding:12px;border-radius:10px;border:1px solid #333;background:#0f0f0f;color:#fff">
          <input id="tplUnit" placeholder="кг / л / шт" style="padding:12px;border-radius:10px;border:1px solid #333;background:#0f0f0f;color:#fff">
          <input id="tplDeadline" type="time" style="padding:12px;border-radius:10px;border:1px solid #333;background:#0f0f0f;color:#fff">
        </div>
        <button id="addTemplateBtn" class="primary" style="width:100%;margin-top:12px;padding:14px;border-radius:12px">+ Добавить</button>
      </div>
    </div>
  `
}
...
export async function initPassportActions(){
  ...
  // слушатели для главных кнопок - они всегда в DOM
...
async function openTemplatesModal(){
  ...
  // КЛЮЧЕВОЙ МОМЕНТ: вешаем слушатели ПОСЛЕ того как модал отрисован
  ...
  modal.addEventListener('click', (e)=>{ if(e.target.id === 'templatesModal') closeTemplatesModal() })
...
function closeTemplatesModal(){
  ...
  // чистим слушатели чтобы не дублировались при повторном открытии
  document.getElementById('closeTemplatesBtn').replaceWith(document.getElementById('closeTemplatesBtn').cloneNode(true))
  document.getElementById('addTemplateBtn').replaceWith(document.getElementById('addTemplateBtn').cloneNode(true))
}
...
async function addTemplate(){
  ...
  if(!title ||!qty ||!restaurantId){
    alert('Заполни название и кол-во');
    return
  }
...
async function loadTemplatesList(){
  ...
  list.innerHTML = data.map(t => `
    <div style="display:flex;justify-content:space-between;align-items:center;padding:14px 16px;background:#27272a;border-radius:14px;border:1px solid #3f3f46">
      <div style="flex:1">
  ...
  // вешаем удаление на каждую кнопку после рендера
...
//... createTodayPrep, loadPrepList, weighTask - оставляем как были
async function createTodayPrep(){
  ...
  const tasks = templates.map(t => ({
    restaurant_id, title: t.title, target_qty: t.target_qty,
    unit: t.unit, deadline: t.deadline, status: 'todo', shift_date: today
  }))
...
async function loadPrepList(){
  ...
  if(error){ document.getElementById('prepList').innerHTML = `<p style="color:#fca5a5">Ошибка: ${error.message}</p>`; return }
  ...
  list.querySelectorAll('.weigh-btn').forEach(btn=>{
    btn.addEventListener('click', ()=>weighTask(btn.dataset.id, btn.dataset.unit))
  })
}
...
async function weighTask(id, unit){
  ...
  await supabase.from('tasks').update({
    status: 'done', actual_qty: parseFloat(qty), staff_id: userId
  }).eq('id', id)
  loadPrepList()
}
